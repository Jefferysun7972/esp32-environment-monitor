...
#include "sen68_i2c.h"    /* ← 添加这一行！提供 SEN66_I2C_ADDR_6B 宏 */
#include <esp_log.h>
...
static const char* TAG = "sen68_i2c_hal";
...
void sensirion_i2c_hal_init(void) {
    ...
    for (uint8_t addr = 0x08; addr <= 0x77; addr++) {
        ...
        if (err == ESP_OK) {
            ...
            if (probe_result == ESP_OK || probe_result == ESP_ERR_TIMEOUT) {
                ...
                if (addr == SEN68_I2C_ADDR_6B) {
                    device_name = "✅ SEN68 Environmental Sensor";
                } else if (addr == 0x23) {
    ...
    if (!dev_handle && bus_handle) {
        i2c_device_config_t sen66_cfg = {
            .dev_addr_length = I2C_ADDR_BIT_LEN_7,
            .device_address = SEN68_I2C_ADDR_6B,
            .scl_speed_hz = I2C_MASTER_FREQ_HZ,
        };
        
        esp_err_t sen66_err = i2c_master_bus_add_device(bus_handle, &sen66_cfg, &dev_handle);
        if (sen66_err == ESP_OK) {
            ESP_LOGI(TAG, "✅ Pre-created SEN68 device handle (address: 0x%02X)", SEN68_I2C_ADDR_6B);
        } else {
            ESP_LOGE(TAG, "Failed to pre-create SEN68 device handle: %s", esp_err_to_name(sen66_err));
        }
    }
    /* ===== 句柄预创建结束 ===== */
