...
static esp_err_t sen6x_write_with_crc(i2c_master_dev_handle_t dev_handle,
                                       uint16_t cmd,
                                       const int16_t *params,
                                       size_t param_count)
{
    size_t total = 2 + param_count * 3;
    uint8_t *tx_buf = malloc(total);
    ...
    esp_err_t err = i2c_master_transmit(dev_handle, tx_buf, total, -1);
    free(tx_buf);
    return err;
}
...
esp_err_t sen6x_init(i2c_master_dev_handle_t dev_handle, sen6x_type_t *out_type)
{
    ...
    // Temperature Compensation (0x60B2) — 不写入
    // SEN68 出厂温度补偿已足够准确，写入自定义值会覆盖出厂默认值，
    // 导致温度读数偏差（实测写入后温度从 23°C 跳变到 50°C）。
    // 如需校准，在稳定环境下测量参考温度后逐步调整 offset 值。

    // VOC Algorithm Tuning Parameters (0x60D0)
    // Order: index_offset, learning_time, learning_gain, gating_max, std_initial, gain_factor
    // learning_time and learning_gain are kept equal per Sensirion SEN5x guidelines.
    // ref: ref_combo.ex @voc_algorithm_tuning_parameters
    int16_t voc_params[6] = {100, 72, 72, 180, 50, 230};
    err = sen6x_write_with_crc(dev_handle, SEN6X_CMD_SET_VOC_TUNING, voc_params, 6);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set VOC tuning params");
        return err;
    }
    ESP_LOGI(TAG, "VOC algorithm tuning parameters set");
